...
def create_LeaveOneOut_instance(gene_indexes_set, cell_count_aa, gene_expression):
    gene_indexes=gene_indexes_set
    y=cell_count_aa
...
def find_parameters_evaluation(index_set,gene_expression, cell_count_aa):
    ...
    loo,X,y = create_LeaveOneOut_instance(index_set, cell_count_aa, gene_expression)
    for train_index, test_index in loo.split(X):
        X_train, X_test = X.iloc[train_index], X.iloc[test_index]
        y_train, y_test = y[train_index], y[test_index]
        # THIS IS INNER LOOP: for TRAINING/VALIDATION
        gam = LinearGAM()
        gam=gam.gridsearch(X_train, y_train,n_splines=np.arange(10,50),lam=[0.4,0.5,0.6,0.7,0.8])
